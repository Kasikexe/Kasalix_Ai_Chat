...
export function ServerConfig({ onSaved, showClose, onClose }: ServerConfigProps) {
 ...
 return (
 <div className="fixed inset-0 z-[10000] bg-gray-950/95 flex items-center justify-center p-4">
 <div className="w-full max-w-md">
 {/* Header */}
 <div className="text-center mb-8">
 <div className="mx-auto w-16 h-16 bg-[#2a3a52] rounded-lg flex items-center justify-center mb-4">
 ...
 ? 'bg-purple-600/30 border border-purple-500/50 text-[#9bb8d6]'
 ...
 {testResult === 'success' && (
 <div className="flex items-center gap-2 px-3 py-2 bg-emerald-900/30 border border-[#1a3a33] rounded-lg text-[#4a9988] text-sm">
 <Check size={16} />
 Server is reachable
 </div>
 )}
 {testResult === 'fail' && (
 <div className="flex items-center gap-2 px-3 py-2 bg-red-900/30 border border-[#5a3030] rounded-lg text-[#c44] text-sm">
 <WifiOff size={16} />
 ...
 onClick={handleSave}
 disabled={saving || !url}
 className="flex-1 px-4 py-2.5 bg-[#2a3a52] hover:bg-[#345070] disabled:opacity-50 rounded-xl text-white text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2"
 >
 {saving ? (
 <Loader2 size={16} className="animate-spin"/>
 ) : (
 ...
 <button
 onClick={onClose}
 className="mt-3 text-[#7b9fc6] hover:text-[#9bb8d6] transition-colors"
 >
 Skip for now
 </button>
 )}
