...
function getFileIcon(name: string, type: 'file' | 'directory') {
 if (type === 'directory') return <Folder size={14} className="text-[#b8966a]"/>;
 ...
 case 'jsx':
 case 'mjs':
 return <FileCode size={14} className="text-[#7b9fc6]"/>;
 case 'json':
...
function DirectoryNode({ name, path, depth, onFileSelect, refreshToken }: {
 ...
 <Folder size={14} className="text-[#b8966a] flex-shrink-0"/>
...
export function FileTree({ rootPath, workspacePath, onFileSelect, onBrowseFolder, refreshToken }: Props) {
 ...
 className="flex items-center justify-center gap-1.5 px-3 py-2 bg-[#1a2a3a] hover:bg-purple-600/30 border border-[#2a3a52] rounded-lg text-xs text-[#9bb8d6] transition-colors mx-auto"
 >
 <FolderOpen size={12} />
 Browse Local Folder
 </button>
 ...
 <FileCode size={12} className="text-[#7b9fc6] flex-shrink-0"/>
 <input
 type="text"
 ...
 {creatingError && (
 <p className="text-[10px] text-[#c44] mt-1 ml-5">{creatingError}</p>
 )}
 </div>
 )}
