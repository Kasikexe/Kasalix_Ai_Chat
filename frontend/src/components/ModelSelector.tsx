...
export function ModelSelector({ models, selected, onChange, getModelStatus }: Props) {
 ...
 currentStatus === 'available' ? 'text-[#4a9988] fill-emerald-400' :
 currentStatus === 'hidden' ? 'text-[#b8966a]' : 'text-[#c44]'
 }`}
 />
 ...
 {open && (
 <div className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto bg-gray-800 border border-gray-700 rounded-lg z-50">
 {models.length === 0 ? (
 ...
 {isCurrent && <span className="ml-auto text-xs text-[#7b9fc6]">current</span>}
 </div>
