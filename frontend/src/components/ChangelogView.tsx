...
const TYPE_STYLES: Record<string, { icon: React.ReactNode; label: string; color: string; gradient: string }> = {
 ...
 label: 'Major',
 color: 'text-[#4a9988] bg-emerald-900/20 border-emerald-700/30',
 gradient: '',
 },
 minor: {
 icon: <Layers size={14} />,
 label: 'Minor',
 color: 'text-[#7b9fc6] bg-[#0e1a28] border-blue-700/30',
 gradient: ' ',
 },
 patch: {
 icon: <AlertTriangle size={14} />,
 label: 'Patch',
 color: 'text-[#b8966a] bg-[#1a1610] border-amber-700/30',
 gradient: '',
 },
};
...
export function ChangelogView({ onClose }: Props) {
 ...
 <GitBranch size={16} className="text-[#7b9fc6]"/>
 ...
 className="text-xs text-[#7b9fc6] hover:text-[#9bb8d6] transition-colors"
 >
 Try again
 </button>
 <button
 ...
 <div className="absolute left-[26px] top-0 bottom-0 w-px bg-[#2a3a52]"/>
 ...
 {isLatest && (
 <span className="text-[9px] font-medium text-[#7b9fc6] bg-purple-900/20 px-1.5 py-0.5 rounded-full">Latest</span>
 )}
 {entry.prerelease && (
 <span className="text-[9px] font-medium text-[#b8966a] bg-[#1a1610] px-1.5 py-0.5 rounded-full">Pre-release</span>
 )}
 </div>
 ...
 <button
 onClick={() => openExternal(RELEASES_URL)}
 className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-[#7b9fc6] transition-colors"
 >
 <ExternalLink size={12} />
