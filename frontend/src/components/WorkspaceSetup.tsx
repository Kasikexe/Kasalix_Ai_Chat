...
export function WorkspaceSetup({ defaultBasePath: propDefault, onSelect, onClose }: Props) {
 ...
 <div className="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-lg overflow-hidden max-h-[80vh] flex flex-col">
 {/* Header */}
 ...
 className={`hover:text-[#7b9fc6] transition-colors ${i === dirPathStack.length - 1 ? 'text-[#7b9fc6] font-medium' : 'text-gray-500'}`}
 >
 {d.name}
 </button>
 </span>
 ...
 <p className="text-xs text-[#c44] mb-2">{error}</p>
 <button
 onClick={() => loadDirs(basePath)}
 className="text-xs text-[#7b9fc6] hover:text-[#9bb8d6] underline"
 >
 Retry
 </button>
 </div>
 ...
 >
 <Folder size={16} className="text-[#b8966a] flex-shrink-0"/>
 ...
 className="text-[11px] text-gray-500 hover:text-[#7b9fc6] transition-colors"
 >
