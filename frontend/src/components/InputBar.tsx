    ...
    >
      ...
      {isDragOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
          <div className="bg-[#0e1a28] border-2 border-dashed border-[#5b7fa6] rounded-lg px-8 py-6">
            <p className="text-blue-200 text-sm font-medium">Drop image to attach</p>
          </div>
        </div>
      )}
      <div className="max-w-3xl mx-auto p-2 md:p-3 relative">
        {image && (
        ...
        {/* Toolbar row — plan mode + tool permission (agent mode only) */}
        {hasToolbar && (
          <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
            ...
            {/* Separator */}
            {onPlanModeChange !== undefined && onToolPermissionChange !== undefined && (
        ...
        {/* Input row */}
        <div className="relative flex items-end gap-1.5 bg-gray-800 border border-gray-700 rounded-lg p-1.5 focus-within:border-gray-600 transition-colors">
          <input
            ref={fileRef}
            type="file"
            accept="image/*"
          ...
          {isStreaming ? (
            <button
              onClick={onStop}
              className="flex-shrink-0 p-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg"
            >
            ...
            <button
              ...
              className="flex-shrink-0 p-1.5 bg-[#2a3a52] hover:bg-blue-700 disabled:bg-gray-700 text-white rounded-lg"
            >
              <Send size={16} />
            </button>
          )}
        ...
        <p className="mt-1.5 text-center text-[11px] text-gray-500 select-none">
