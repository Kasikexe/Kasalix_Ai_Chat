...
function ChatApp({ user, onSwitchUser, thinkingEnabled, onToggleThinking }: ChatAppProps) {
 ...
 const [mode, setMode] = useState<ConversationMode>('chat');  const [viewTab, setViewTab] = useState<'chat' | 'agent'>('chat');
 ...
 // Cloud unavailable notification — listen for the custom event from useChat
 const toastHook = useToast();
 useEffect(() => {
 const handler = () => toastHook.warning('Cloud provider unavailable. Falling back to local models.');
 window.addEventListener('cloud-unavailable', handler);
 return () => window.removeEventListener('cloud-unavailable', handler);
 }, [toastHook]);
 ...
 // Agent modes are only available in the desktop app
 const isElectron = typeof window !== 'undefined' && !!(window as any).electronAPI?.isElectron;  if (!isElectron && newMode === 'agent') return;
 setMode(newMode);
 setViewTab(newMode);
 setActiveId(null);
 };
